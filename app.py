...
st.title("🚢 DEĞİŞKEN DENİZ DURUMLARINA GÖRE YAKIT TÜKETİMİ TAHMİNİ VE OPTİMİZASYONU")
st.markdown("**Modüller:** **Computer Vision** | Big Data & CII | Live Satellite | Geospatial Routing | Storm Radar")
st.markdown("---")
...
def calculate_instant_fuel(speed, draft, wind_area, beaufort, des_spd, des_dft, des_cons):
    base_cons = des_cons * (speed / des_spd)**3 * (draft / des_dft)**(2/3)
    wind_speed_ms = 0.836 * (beaufort ** 1.5)
    v_ship_ms = speed * 0.5144
    v_rel_ms = v_ship_ms + wind_speed_ms 
    r_air_newton = 0.5 * 1.225 * 0.8 * wind_area * (v_rel_ms ** 2)
    p_air_kw = (r_air_newton * v_ship_ms) / 1000
    wave_height = 0.1 * (beaufort ** 2)
    est_power_kw = (base_cons * 1000000) / (24 * 180) 
    p_wave_kw = est_power_kw * (0.05 * wave_height)
    sfoc = 180
    return base_cons + ((p_air_kw + p_wave_kw) * sfoc * 24 / 1000000)
...
st.sidebar.markdown("---")
st.sidebar.header("📡 2. Uydu Bağlantısı")
api_key_global = st.sidebar.text_input("OpenWeather API Key:", type="password")

ref_cii = (d_cons * 3.114 * 1_000_000) / (dwt * d_speed * 24)

# =============================================================================
# BÖLÜM 3: OPENCV RÜZGAR ALANI HESAPLAYICI (YENİ EKLENDİ)
# =============================================================================
st.header("📷 Rüzgar Alanı Analizi")
st.write("Geminin ön cephe (transvers) fotoğrafını yükleyerek rüzgar direncine maruz kalan alanı otomatik hesaplayın. Bu değer tüm yakıt hesaplarına yansıyacaktır.")

with st.expander("Geminin Transvers (Ön) Cephe Fotoğrafını Yükle", expanded=False):
    col_cv1, col_cv2 = st.columns(2)
    uploaded_file = col_cv1.file_uploader("Gemi Fotoğrafı (Siyah-Beyaz veya Beyaz Arkaplanlı tercih edilir)", type=['png', 'jpg', 'jpeg'])
    ...
    if uploaded_file is not None:
        ...
        if contours:
            ...
            # Pikselleri Gemi Genişliğine (Beam) göre oranlayıp metrekare bulma
            ...
            col_cv2.success(f"✅ Gemi Genişliği (Beam): {beam} m\n\n✅ Tespit Edilen Piksel Genişliği: {w} px\n\n🌬️ **Hesaplanan Transvers Rüzgar Alanı:** {real_area:.1f} m²\n\n*(Bu değer tüm simülasyona aktarıldı)*")
        else:
            col_cv2.error("Gemi silueti bulunamadı. Lütfen daha net bir fotoğraf yükleyin.")
            st.session_state.calc_wind_area = col_cv2.number_input("Manuel Rüzgar Alanı (m²)", value=st.session_state.calc_wind_area)
    else:
        st.session_state.calc_wind_area = col_cv2.number_input("Manuel Rüzgar Alanı (m²) - Fotoğraf yüklemediyseniz", value=st.session_state.calc_wind_area)
...
with st.expander("📊 Sanal Sefer Verisi Üret ve Analiz Et", expanded=False):
    ...
    with col_sim1:
        ...
        # OpenCV'den gelen veriyi varsayılan yaptık
        ...
        if st.button("🚀 Veri Setini Üret"):
            with st.spinner(f"Arka planda {sim_days} günlük okyanus simülasyonu çalıştırılıyor..."):
                ...
                for i in range(sim_days):
                    fuel = calculate_instant_fuel(sim_speeds[i], sim_drafts[i], sim_wind_area, sim_bfts[i], d_speed, d_draft, d_cons)
    ...
    if 'df_sim' in st.session_state:
        ...
        fig_scatter.update_layout(
            title='Hız ve Yakıt Tüketimi (IMO Çevresel Performans Karnesine Göre)',
            xaxis_title='Gemi Hızı (Knot)', yaxis_title='Günlük Yakıt Tüketimi (Ton)',
            height=500, hovermode='closest'
        )
        st.plotly_chart(fig_scatter, use_container_width=True)

        st.markdown("---")
        st.markdown("### 🏆 Tüm Simülasyonun Genel Çevresel Karnesi")
        ...
        c_gen3.markdown(f"""
        <div style="background-color: {g_color}; padding: 15px; border-radius: 10px; text-align: center; box-shadow: 0 4px 8px rgba(0,0,0,0.1);">
            <h4 style="margin: 0; padding: 0; color: white; font-weight: normal;">GENEL IMO KARNESİ</h4>
            <h1 style="margin: 0; padding: 0; color: white; font-size: 40px; font-weight: bold;">{overall_grade}</h1>
            <p style="margin: 0; padding: 0; color: white; font-size: 14px;">Ortalama CII: {overall_cii:.2f}</p>
        </div>
...
# =============================================================================
# BÖLÜM 5: GLOBAL NAVİGASYON (SEAROUTE ENTEGRASYONU & FIRTINA RADARI)
# =============================================================================
st.header("🗺️ Global Geospatial Navigasyon")
...
ports = {
    ...
    "Los Angeles": (34.0, -118.2), "Cape_Town": (-33.9, 18.4), "Santos": (-23.9, -46.3)
}
...
opt_priority = st.slider(
    ...
    help="0 = Yakıt Tasarrufu İçin Yavaşla (Eco-Speed). 100 = Fırtınada Bile Tam Yol İleri (Zaman Önceliği)."
)

if st.button("📡 Kusursuz Okyanus Rotasını Çiz ve Analiz Et"):
    with st.spinner("Uydu verileri ve Kıyı Şeritleri analiz ediliyor..."):
        
        origin = [ship_lon, ship_lat]
        destination = [dest_lon, dest_lat]
        
        try:
            route = sr.searoute(origin, destination)
            if route is None:
                st.error("Kütüphane bu iki nokta arasında rota bulamadı. Koordinatların karada olmadığından emin olun.")
                st.stop()
            route_coords = route["geometry"]["coordinates"] 
        except Exception as e:
            st.error(f"🚨 SİSTEM HATASI DETAYI: {str(e)}")
            st.stop()
        ...
        for i in range(len(route_coords)-1):
            ...
            total_distance_nm += haversine_distance(lat1, lon1, lat2, lon2)
        ...
        if api_key_global:
            bft_sum = 0
            valid_samples = 0
            my_bar = st.progress(0.0, text="Okyanus fırtınaları taranıyor...")
            ...
            if valid_samples > 0:
                avg_bft = round(bft_sum / valid_samples)
            my_bar.empty()
        else:
        ...
        # OpenCV'den gelen Rüzgar Alanı kullanılıyor!
        w_area = st.session_state.calc_wind_area
        
        daily_fuel = calculate_instant_fuel(chosen_speed, d_draft, w_area, avg_bft, d_speed, d_draft, d_cons)
        days_on_route = total_distance_nm / (chosen_speed * 24)
        total_fuel = daily_fuel * days_on_route

        bad_daily_fuel = calculate_instant_fuel(max_speed, d_draft, w_area, avg_bft, d_speed, d_draft, d_cons)
        ...
        for seg in route_segments:
            ...
            fig.add_trace(go.Scattergeo(
                lon=lon_list, lat=lat_list, mode='lines',
                line=dict(width=4, color=seg['color']),
                hoverinfo='text',
                text=f"Bölge Havası: {seg['bft']} Bft",
                showlegend=False
            ))
        ...
        ship_trace = go.Scattergeo(
            lon=[ship_lon], lat=[ship_lat], mode='markers+text',
            marker=dict(size=14, color='orange', symbol='triangle-up'),
            text=["📍 ÇIKIŞ"], textposition="top right", name="Gemi Konumu"
        )
        dest_trace = go.Scattergeo(
            lon=[dest_lon], lat=[dest_lat], mode='markers+text',
            marker=dict(size=14, color='blue', symbol='star'),
            text=[f"🏁 {dest_port}"], textposition="bottom center", name="Varış Limanı"
        )

        fig.add_trace(ship_trace)
        fig.add_trace(dest_trace)

        fig.update_layout(
            title_text=f'Gerçek Deniz Yolu Navigasyonu (Ortalama Hava: {avg_bft} Bft)',
            showlegend=True,
            geo=dict(showland=True, landcolor="rgb(243, 243, 243)", showocean=True, oceancolor="rgb(204, 229, 255)", showcountries=True, countrycolor="rgb(204, 204, 204)", projection_type="equirectangular"),
        ...
        fig_bar = go.Figure(data=[
            go.Bar(name='Yakıt Tüketimi (Ton)', 
                   x=['Maksimum Hız (Tam Yol)', 'AI Optimize Seçim'], 
                   y=[bad_total_fuel, total_fuel],
                   text=[f"{bad_total_fuel:,.0f} Ton", f"{total_fuel:,.0f} Ton"],
                   textposition='auto',
        ...
        fuels_array = []
        for s in speeds_array:
            dfuel = calculate_instant_fuel(s, d_draft, w_area, avg_bft, d_speed, d_draft, d_cons)
            tfuel = dfuel * (total_distance_nm / (s * 24))
            fuels_array.append(tfuel)
            
        fig_curve = go.Figure()
        
        fig_curve.add_trace(go.Scatter(
            x=speeds_array, y=fuels_array, 
            mode='lines', name='Hız-Yakıt Karakteristiği', 
            line=dict(color='#3498db', width=3)
        ))
        
        fig_curve.add_trace(go.Scatter(
            x=[max_speed], y=[bad_total_fuel], 
            mode='markers+text', name='Geleneksel Seyir', 
            marker=dict(color='#e74c3c', size=12, symbol='x'), 
            text=['❌ Tam Yol'], textposition='top right', textfont=dict(color='#c0392b', size=11)
        ))

        fig_curve.add_trace(go.Scatter(
            x=[chosen_speed], y=[total_fuel], 
            mode='markers+text', name='AI Optimizasyonu', 
            marker=dict(color='#2ecc71', size=14, line=dict(color='white', width=2)), 
            text=['✅ AI Seçimi'], textposition='bottom right', textfont=dict(color='#27ae60', size=13)
        ))
        
        fig_curve.add_trace(go.Scatter(
            x=[chosen_speed, chosen_speed, max_speed], 
            y=[total_fuel, bad_total_fuel, bad_total_fuel], 
            mode='lines', name='Tasarruf Alanı', 
            line=dict(color='gray', width=2, dash='dash'),
            showlegend=False
        ))
        
        fig_curve.add_annotation(
            x=(chosen_speed + max_speed)/2, y=bad_total_fuel,
            text=f"Tasarruf: {bad_total_fuel - total_fuel:,.0f} Ton",
            showarrow=True, arrowhead=2, ax=0, ay=-30,
            font=dict(color="green", size=13)
        )
        
        fig_curve.update_layout(
            title='Yapay Zeka Dinamik Hız Optimizasyonu (Kübik Eğri)', 
            xaxis_title='Gemi Hızı (Knot)', 
            yaxis_title='Toplam Sefer Yakıtı (Ton)', 
            height=400,
            hovermode="x unified"
        )
